...
import {
  SubAccountDetailsSchema,
  TSubAccountDetailsSchema,
} from '@/schemas/subaccount-details.schema';
...
const SubAccountDetails: React.FC<SubAccountDetailsProps> = ({
  ...
  const form = useForm<TSubAccountDetailsSchema>({
  ...
  async function onSubmit(values: TSubAccountDetailsSchema) {
    startTransition(async () => {
      try {
        const mongoId = await generateRandomMongoId();

        const response = await upsertSubAccount({
          id: details?.id ? details.id : mongoId,
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className='space-y-4'>
            ...
            <Button type='submit' disabled={pending}>
              {pending ? <Loading /> : 'Save Account Information'}
            </Button>
          </form>
        </Form>
      </CardContent>
